...
Текущие версии: `identity-v4` для группировки и `market-v4` для оценки рынка.
...
1. подтверждённое пользователем объединение;
2. активное model rule;
3. точная нормализованная сигнатура `brand + product family + model tokens`;
4. source product id;
5. отдельная группа объявления.
...
Пограничные пары внутри одного бренда и product family создаются как
`identity_matches(level=model, status=pending)`. Автослияние такой пары возможно
только при сходстве title не ниже 92 и совпавшем/почти совпавшем image fingerprint;
остальные пары ждут подтверждения. Решение пользователя имеет приоритет при
следующих пересчётах.
